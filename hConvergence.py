...
'''
This code corresponds to Example 1 (Tables 1 & 2) of the [arXiv preprint](https://arxiv.org/abs/2404.13578)

Tested with NGSolve version 6.2.2404

h-Convergence test for fully transient poroelasticity problem
...
def weaksym(k, h, dt, tend):
    ...
    s = 1
    #Lamé coef. corresponding to C (Table 1)
    mu  = 50 
    lam = 100
    
    #Constrained specific storage (Table 2)
    #s = 1e-4
    #Lamé coef. corresponding to C (Table 2) 
    #lam = 1e8
    ...
    fes = FESpace([S, S, S, S, W, W, hatS, hatF])
    ...
    while t_intermediate < tend:
        ...
        u0.vec[:] = 0.0 
        u0.components[6].Set(exactSv, BND)
...
dt = (1/64)**(pp/2+1) # Time step
tend = 0.3 # Final time


er_s, er_u = collecterrors(pp, maxlevels, dt, tend)
